""" Tests dice_scrape.py.
    testpytest.py rev 2013 June 22 second version of day, by Stuart Ambler.
    Copyright (c) 2013 Stuart Ambler.
    Distributed under the Boost License in the accompanying file LICENSE.
    Usage:  python -m py.test testpytest.py
...
class TestDiceScrape:
    ...
    def testhtml (self, capsys):
        ...
        assert self.compareouterr (capsys, "testout", "testerr")
        
    def testhtmlnojobnrs (self, capsys):
        main_args = ["./dice_scrape.py",
                     "http://localhost:8000/testnojobnrs.html", "test"]
        dice_scrape.main_func (main_args)
        assert self.compareouterr (capsys, "testoutnojobnrs", "testerrnojobnrs")

    def testnonexistent (self, capsys):
        main_args = ["./dice_scrape.py",
                     "http://localhost:8000/testnonexistent.html", "test"]
        dice_scrape.main_func (main_args)
        assert self.compareouterr (capsys, "testoutnonexistent",
                                   "testerrnonexistent")
